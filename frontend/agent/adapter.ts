...
class AgentAdapterService {
    ...
    // 1. ADD THIS PROPERTY to track context 🧠
    private sessionType: 'CHECK_IN' | 'NEW_BOOKING' = 'NEW_BOOKING';
    ...
    // 1. CALL THE BRAIN 🧠
    private async askTheBrain(text: string): Promise<{ intent: string, payload?: any, message?: string }> {
        try {
            // A. Show "Thinking" State (Optional)
            ...
            const response = await fetch('http://localhost:3002/api/chat', {
                ...
                body: JSON.stringify({
                    message: text,
                    context: { screen: this.state } // Send current screen so AI has context
                })
            });

            const data = await response.json();
            return data; // Returns { intent: "ROOM_SELECTED", payload: { ... } }
    ...
    public async processWithLLMBrain(transcript: string, sessionId?: string): Promise<void> {
        ...
        // 🧠 LOCAL KEYWORD SPOTTER (The "Micro-Brain")
        // Check for specific state overrides before calling the expensive LLM
        if (this.state === 'CONFIRM_BOOKING') {
            const lower = transcript.toLowerCase();
            if (lower.includes('yes') || lower.includes('confirm') || lower.includes('correct') || lower.includes('proceed')) {
                console.log("[Agent] 🧠 Local Override: 'CONFIRMED'");
                this.dispatch('CONFIRMED');
                return;
            }
            if (lower.includes('no') || lower.includes('wrong') || lower.includes('change')) {
                console.log("[Agent] 🧠 Local Override: 'CANCEL_REQUESTED'");
                this.dispatch('CANCEL_REQUESTED');
                return;
            }
        }

        // C. Get Intelligent Decision
        const interpretation = await this.askTheBrain(transcript);

        // B. Handle Confusion / Greetings
        if (interpretation.intent === 'UNKNOWN') {
            // Did the brain send a custom help message?
            if (interpretation.message) {
                this.speak(interpretation.message);
            } else {
                this.speak("I didn't quite catch that. You can say 'Check In' or 'Book a Room'.");
            }
            return;
        }

        // D. Execute
        console.log(`[Agent] Executing: ${interpretation.intent}`, interpretation.payload);

        this.dispatch(interpretation.intent as Intent, interpretation.payload);
    }
    ...
    public handleIntent(intent: string, payload?: any) {
        ...
        this._processIntent(intent, payload);
    }

    /**
     * Core intent processing logic (Shared by Touch & Voice)
     */
    private _processIntent(intent: string, payload?: any) {
        console.log(`[Agent] Processing Intent: ${intent}`);

        // 1. TRACK SESSION TYPE 🧠
        if (intent === 'CHECK_IN_SELECTED') {
            this.sessionType = 'CHECK_IN';
        } else if (intent === 'BOOK_ROOM_SELECTED') {
            this.sessionType = 'NEW_BOOKING';
        }

        // 2. SMART ROUTING AT SCAN COMPLETION 🔀
        if (intent === 'SCAN_COMPLETED') {
            if (this.sessionType === 'CHECK_IN') {
                console.log("✅ Check-In Mode: Found Reservation.");
                this.speak("I have found your reservation. Please confirm details.");

                // FORCE TRANSITION TO 'CONFIRM_BOOKING'
                // We manually feed the 'SCAN_COMPLETED_EXISTING' event to the machine
                const nextState = StateMachine.transition(this.state as UIState, 'SCAN_COMPLETED_EXISTING') as UiState;
                this.transitionTo(nextState);
                return;
            }
            else {
                console.log("🆕 New Booking Mode: Select Room.");
                this.speak("Thank you. Please select your room.");

                // FORCE TRANSITION TO 'ROOM_SELECT'
                const nextState = StateMachine.transition(this.state as UIState, 'SCAN_COMPLETED_NEW') as UiState;
                this.transitionTo(nextState);
                return;
            }
        }

        // 3. DEFAULT TRANSITION (The normal path)
        ...
        if (nextState !== this.state) {
            ...
            // 4. SPEAK IF NEEDED
            if (nextState === 'PAYMENT' && intent === 'CONFIRMED') {
                this.speak("Processing payment. Please wait...");
            }

            // If we just moved to manual menu, make sure we speak
            if (nextState === 'MANUAL_MENU' && intent !== 'VOICE_SESSION_ENDED' && intent !== 'VOICE_TRANSCRIPT_PARTIAL') {
                this.speak("Please select an option from the menu.");
            }

            // If we just moved to SCAN_ID, prompt the user
            if (nextState === 'SCAN_ID' && intent !== 'CHECK_IN_SELECTED' && intent !== 'BOOK_ROOM_SELECTED') {
                this.speak("Please scan your ID or passport.");
            }

            // 5. HARDWARE SIMULATION (Key Dispenser) 🔑
            if (nextState === 'KEY_DISPENSING') {
                console.log("⚙️ Starting Hardware Simulation...");
                this.speak("Please wait while I prepare your key.");

                // START THE TIMER (3 Seconds) ⏳
                setTimeout(() => {
                    console.log("✅ Hardware: Key Dispensed Successfully.");

                    // A. Speak the Final Success Message
                    this.speak("Here is your key card. Room 404 is on the 4th floor. Enjoy your stay.");

                    // B. Move State Machine to Finish
                    // We fire the 'DISPENSED' event we just added
                    // Using _processIntent to handle the transition logic (and recursion safety?)
                    // _processIntent is safe here as it's an internal event
                    this._processIntent('DISPENSED');

                    // C. Optional: Log the completion for analytics
                    console.log("🏁 Session Complete. Resetting to IDLE.");
    ...
    public dispatch(intent: Intent, payload?: any) {
        ...
        // We reuse the calculation logic from _processIntent, skipping the "Kill Audio" part.
        this._processIntent(intent, payload);
    }
