...
export const VoiceStatusIndicator: React.FC<VoiceStatusIndicatorProps> = ({ currentState, voiceEnabled }) => {
    ...
    return (
        <button
            ...
            className={`fixed left-4 bottom-4 z-40 pointer-events-auto rounded-full border px-3 py-2 text-xs font-medium shadow-lg backdrop-blur-md transition-colors ${badgeClassByKind[status.kind]} ${canTapToToggle ? "hover:brightness-110" : "cursor-not-allowed opacity-85"}`}
            aria-live="polite"
