...
const ParticleWave: React.FC<ParticleWaveProps> = ({ className = '' }) => {
  ...
  const getBackgroundColor = (theme: string) => {
    return theme === 'dark' 
      ? new THREE.Color(0x0f172a) // Match Slate 900
  ...
  const initScene = () => {
    ...
    // We want transparent background for the canvas so CSS bg shows through or handles it
    // But the component code sets clear color. Let's use the theme function.
