...
export function CardStack<T extends CardStackItem>({
  ...
  }, [autoAdvance, reduceMotion, len, pauseOnHover, hovering, loop, active, intervalMs, next]);
  ...
  return (
    ...
    >
      <div className="relative w-full" style={{ height: Math.max(560, cardHeight + 120) }} tabIndex={0} onKeyDown={onKeyDown}>
        <div
          className="pointer-events-none absolute inset-x-0 top-6 mx-auto h-48 w-[70%] rounded-full bg-black/5"
          aria-hidden="true"
        />
        <div
        ...
        <div className="absolute inset-0 flex items-center justify-center" style={{ perspective: `${perspectivePx}px` }}>
          <AnimatePresence initial={false}>
            {items.map((item, i) => {
              ...
              const isOppositeCardOnEvenLoop = loop && len % 2 === 0 && abs === len / 2;
              if (!visible || isOppositeCardOnEvenLoop) return null;
              ...
              const isSingleVisible = maxOffset === 0;
              const scale = isActive ? activeScale : inactiveScale;
              const lift = isActive ? -activeLiftPx : 0;
              const rotateX = isActive ? 0 : tiltXDeg;
              const zIndex = 100 - abs;
              const singleEntryX = direction > 0 ? cardWidth * 0.85 : -cardWidth * 0.85;
              const singleExitX = direction > 0 ? -cardWidth * 0.85 : cardWidth * 0.85;
                ...
                <motion.div
                  ...
                  initial={
                    reduceMotion
                      ...
                      : {
                          opacity: 0,
                          y: y + 40,
                          x: isSingleVisible && isActive ? singleEntryX : x,
                          rotateZ,
                          rotateX,
                          scale,
                        }
                  ...
                  exit={
                    reduceMotion
                      ...
                      : {
                          opacity: 0,
                          x: isSingleVisible ? singleExitX : x,
                          y: y + 40,
                          rotateZ,
                          rotateX,
                          scale,
    ...
    <div className="relative h-full w-full">
      <div className="absolute inset-0">
        {item.imageSrc ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-slate-800 text-sm text-slate-300">No image</div>
        )}
      </div>
